...
It's 2025, and I'm still using [Vim](https://www.vim.org)! How about you? If you haven't tried it yet, you're not living
dangerously enough. Some workflows still call for editing files directly on a remote server or container, or you might
find yourself managing code on a tiny Raspberry Pi tucked in your corner. And that's when Vim might come in handy.

I first tried it at 17. I know it can be intimidating and frustrating at first, watching true masters in action was
...
By the way, these commands also work in `vi`, which comes pre-installed on many Linux distros.
...
<Callout description="Search a string">
`Esc` and then `/` followed by the search term
</Callout>

For example, to search for the string "_cache", type `/_cache` and press `Enter`.

<Callout description="Jump to the next occurrence">
`n`
</Callout>

By default, _searches are case sensitive_.

I managed for years with just these commands. The truth is, after a while, Vim can get addictive. It's amazing how much
you can do once you get the hang of it.

There's so much more to learn, but plenty of excellent guides already exist—I have no intention of repeating what's been
explained far better elsewhere. Instead, let's focus on my setup.
...
There are a few plugin managers for Vim. We'll use [vim-plug](https://github.com/junegunn/vim-plug). Install it with:
...
To close each pane, use our beloved `:q` command.

## Explore

Congratulations! You survived Vim and lived a little dangerously.

Although nearly as old as I am, Vim is still very much alive, and for good reasons. It's powerful, flexible and remains
essential even to those who prefer modern IDEs, like me.
